from django.urls import path
from main.views import (
    show_main, create_product, show_product, show_xml, show_json, 
    show_xml_by_id, show_json_by_id, register, login_user, logout_user,
    edit_product, delete_product, add_product_entry_ajax, 
    edit_product_entry_ajax, delete_product_entry_ajax, show_json_by_user,
    login_user_ajax, register_ajax, logout_flutter, create_product_flutter
)

app_name = 'main'

urlpatterns = [
    # Web HTML
    path('', show_main, name='show_main'),
    path('register/', register, name='register'),
    path('login/', login_user, name='login'),
    path('logout/', logout_user, name='logout'),
    path('create-product/', create_product, name='create_product'),
    path('product/<str:id>/', show_product, name='show_product'),
    path('product/<uuid:id>/edit', edit_product, name='edit_product'),
    path('product/<uuid:id>/delete', delete_product, name='delete_product'),
    
    # Data XML/JSON standard
    path('xml/', show_xml, name='show_xml'),
    path('json/', show_json, name='show_json'),
    path('xml/<str:product_id>/', show_xml_by_id, name='show_xml_by_id'),
    path('json/<str:product_id>/', show_json_by_id, name='show_json_by_id'),
    path('show_json_by_user/<int:user_id>/', show_json_by_user, name='show_json_by_user'),
    
    # Web AJAX
    path('create-product-ajax', add_product_entry_ajax, name='add_product_entry_ajax'),
    path('edit-product-ajax/<uuid:id>', edit_product_entry_ajax, name='edit_product_entry_ajax'),
    path('delete-product-ajax/<uuid:id>', delete_product_entry_ajax, name='delete_product_entry_ajax'),

    # API Flutter (JSON)
    path('auth/login/', login_user_ajax, name='login_ajax'),     
    path('auth/register/', register_ajax, name='register_ajax'), 
    path('auth/logout/', logout_flutter, name='logout_flutter'), 
    path('create-flutter/', create_product_flutter, name='create_product_flutter'),
]